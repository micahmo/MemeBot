...
import telepot
from telepot.namedtuple import InlineQueryResultCachedPhoto, InlineQueryResultCachedDocument, KeyboardButton, ReplyKeyboardMarkup, ReplyKeyboardRemove
import pprint
import time
...
def handleInline(msg):
    ...
    for (fileId, relevancy) in fileIdsToSortedRelevancy:
        ...
        if ("photo" in file.get("file_path")):
            ...
            # it's a gif
            photos.append(InlineQueryResultCachedDocument(id=fileId, document_file_id=fileId, title=meme_data[fileId].name))
...
class Meme:
    ...
    def __str__(self):
        return "name: {}, type: {}, fileId: {}, submitter: {}, submitterUsername: {}".format(self.name, self.fileId, self.submitter, self.submitterUsername)
